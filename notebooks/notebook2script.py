...
def notebook2script(fname=None, allFiles=None, upTo=None, destName=None):
    ...
    if not allFiles: notebook2scriptSingle(fname, destName)
    else:
        print('Begin...')
...
def notebook2scriptSingle(fname, dest_name=None):
    "Finds cells starting with `#export` and puts them into a new module"
    fname = Path(Path(fname).absolute())
    fname_out = f'{fname.stem.split("_")[0]}.py' if dest_name is None else f'{dest_name}.py'
...
'''
    ...
    fname_dir_rel_to_notebooks = fname.parent.relative_to(notebooks_root)
    op_dir = lib_root / fname_dir_rel_to_notebooks
    if not op_dir.exists(): op_dir.mkdir(parents=True)
    output_path = op_dir / fname_out
